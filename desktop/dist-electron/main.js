  ...
  let AppUpdater$1 = class AppUpdater2 extends events_1.EventEmitter {
    ...
    async executeDownload(taskOptions) {
      ...
      const done = async (isSaveCache) => {
        ...
        const currentBlockMapFile = path$1.join(cacheDir, "current.blockmap");
        if (await (0, fs_extra_1.pathExists)(currentBlockMapFile)) {
          await (0, fs_extra_1.copyFile)(currentBlockMapFile, path$1.join(downloadedUpdateHelper.cacheDir, "current.blockmap"));
        }
    ...
    async differentialDownloadInstaller(fileInfo, downloadUpdateOptions, installerPath, provider, oldInstallerFileName) {
      try {
        ...
        const saveBlockMapToCacheDir = async (blockMapData, cacheDir) => {
          const blockMapFile = path$1.join(cacheDir, "current.blockmap");
...
function createPresentationWindow(displayId) {
  ...
  presentationWindow = new require$$1.BrowserWindow({
    x,
    y,
    width,
    height,
...
require$$1.ipcMain.handle("get-displays", () => {
  const displays = require$$1.screen.getAllDisplays();
  const primary = require$$1.screen.getPrimaryDisplay();
...
require$$1.ipcMain.handle("open-presentation", (_event, displayId) => {
  if (presentationWindow) {
...
require$$1.ipcMain.handle("close-presentation", () => {
  if (presentationWindow) {
    presentationWindow.close();
    return { success: true };
  }
...
require$$1.ipcMain.handle("update-presentation", (_event, slideData) => {
  if (presentationWindow) {
...
require$$1.ipcMain.handle("is-presentation-open", () => {
  return presentationWindow !== null;
});
require$$1.ipcMain.handle("songs:getAll", () => {
...
require$$1.ipcMain.handle("songs:getById", (_event, id) => {
...
require$$1.ipcMain.handle("songs:create", (_event, song) => {
  const dbSong = {
    ...song,
...
require$$1.ipcMain.handle("songs:update", (_event, id, updates) => {
...
require$$1.ipcMain.handle("songs:delete", (_event, id) => {
  return deleteSong(id);
});
require$$1.ipcMain.handle("schedules:getAll", () => {
...
require$$1.ipcMain.handle("schedules:getById", (_event, id) => {
...
require$$1.ipcMain.handle("schedules:create", (_event, schedule) => {
  return createSchedule(schedule);
});
require$$1.ipcMain.handle("schedules:update", (_event, id, updates) => {
  return updateSchedule(id, updates);
});
require$$1.ipcMain.handle("schedules:delete", (_event, id) => {
  return deleteSchedule(id);
});
require$$1.ipcMain.handle("scheduleItems:add", (_event, item) => {
  return addScheduleItem(item);
});
require$$1.ipcMain.handle("scheduleItems:update", (_event, id, updates) => {
  return updateScheduleItem(id, updates);
});
require$$1.ipcMain.handle("scheduleItems:delete", (_event, id) => {
  return deleteScheduleItem(id);
});
require$$1.ipcMain.handle("scheduleItems:reorder", (_event, scheduleId, itemIds) => {
  reorderScheduleItems(scheduleId, itemIds);
  return true;
});
function ensureBackgroundsDir() {
  if (!fs$1.existsSync(backgroundsDir)) {
...
require$$1.ipcMain.handle("backgrounds:import", async () => {
...
require$$1.ipcMain.handle("ndi:getStatus", () => {
  return getNdiOutput().getStatus();
});
require$$1.ipcMain.handle("ndi:start", async (_event, sourceName) => {
...
require$$1.ipcMain.handle("ndi:stop", () => {
  getNdiOutput().stop();
  return { success: true };
});
require$$1.ipcMain.handle("ndi:sendFrame", (_event, frameData) => {
  const ndi = getNdiOutput();
...
require$$1.ipcMain.handle("ndi:setSourceName", (_event, name) => {
  getNdiOutput().setSourceName(name);
  return { success: true };
});
require$$1.ipcMain.handle("api:fetch", async (_event, options) => {
  try {
    const { url, method = "GET", headers = {}, body } = options;
    const response = await require$$1.net.fetch(url, {
      method,
      headers,
      body: body || void 0
    });
    ...
    return {
      ok: false,
      status: 0,
      statusText: error2 instanceof Error ? error2.message : "Unknown error",
      data: null
    };
  }
});
require$$1.ipcMain.handle("window:focus", () => {
  if (mainWindow) {
...
require$$1.app.whenReady().then(() => {
  require$$1.protocol.handle("app-bg", (request) => {
    ...
    return require$$1.net.fetch(require$$2$1.pathToFileURL(filePath).toString());
  });
  initDatabase();
  ensureBackgroundsDir();
  createMainWindow();
...
require$$1.ipcMain.handle("install-update", () => {
